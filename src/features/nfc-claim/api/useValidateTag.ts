...
/**
 * POST /api/v1/claims/:tagId — validates the scanned tag and returns the product
 * plus which screen to show (CLAIMABLE / ALREADY_CLAIMED_BY_YOU /
...
export function useValidateTag(tagId: string | undefined, enabled = true) {
    return useQuery({
        ...
        queryFn: () => nfcApi.post<ValidateResult>(CLAIM_ROUTES.validate(tagId!)),
        enabled: !!tagId && enabled,
        staleTime: 0, // ownership can change under us
        retry: false, // surface 404 immediately
    })
}
